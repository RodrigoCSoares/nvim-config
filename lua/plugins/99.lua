return {
  ...
  config = function()
    ...
    vim.keymap.set("v", "<leader>9v", function()
      _99.visual()
    end, { desc = "Send visual selection to AI" })

    --- if you have a request you dont want to make any changes, just cancel it
    vim.keymap.set("n", "<leader>9x", function()
      _99.stop_all_requests()
    end, { desc = "Cancel/stop all pending requests" })
    ...
    vim.keymap.set("n", "<leader>9b", function()
      _99.vibe()
    end, { desc = "Vibe coding" })

    --- Worker: set the current work description via vim.ui.input (noice-compatible)
    vim.keymap.set("n", "<leader>9w", function()
      vim.ui.input({ prompt = "99 Work Description: " }, function(input)
        if input and input ~= "" then
          _99.Extensions.Worker.set_work({ description = input })
          print("99: work set to: " .. input)
        end
      end)
    end, { desc = "Set worker description" })

    --- Worker: search for what is left to be done for the current work item
    vim.keymap.set("n", "<leader>9W", function()
      _99.Extensions.Worker.search()
    end, { desc = "Worker search (find remaining work)" })
  end,
}
